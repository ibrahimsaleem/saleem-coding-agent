...
describe('rewriteMarkdown', () => {
  ...
  it('routes a cross-page link to its manifest route within the same locale', () => {
    const { root, pages } = fixture()
    expect(rewriteMarkdown('[B](b.md)\n', {
      locale: 'root',
      sourcePath: 'docs/a.md',
...
describe('docsPages locale routes', () => {
  ...
  it('publishes every route in both route trees from the same English source', () => {
  ...
  it('projects every published subsystem page from its English source in both trees', () => {
  ...
  it('publishes the Cordis core API under matching route structures', () => {
  ...
  it('includes persistence event headings in both route outlines', () => {
  ...
  it('projects reviewed generated pages into both route trees', () => {
    const routes = [
...
describe('llmsTxt', () => {
  ...
  it('groups the two route trees under their own headings', () => {
    const text = llmsTxt(site)
    expect(text.indexOf('## Docs')).toBeGreaterThan(-1)
    expect(text.indexOf('## English')).toBeGreaterThan(text.indexOf('## Docs'))
  })
