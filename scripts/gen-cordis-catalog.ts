...
/** Retained for API stability; the repo is English-only, so a region is emitted verbatim. */
export function localizePageRegion(region: string, _pageRel: string, _scanRoot: string = root): string {
  return region
}
...
export function main(): void {
  ...
  for (const [out] of outputs) {
    try {
      ...
      // First generation of this artifact; nothing to guard.
    }
  }
  let changedPages = 0
  for (const [out, content] of outputs) {
