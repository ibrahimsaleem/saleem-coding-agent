...
export function resolveProfileDir(name: string, home: string = resolveDshHome()): string {
  if (name === '' || name.includes('/') || name.includes('\\') || name === '.' || name === '..'
    ...
    || name === 'node_modules') {
    throw new Error(`saleem: invalid profile name ${JSON.stringify(name)}`)
  }
...
function ensureSymlink(link: string, target: string): void {
  ...
  if (stat !== undefined) {
    if (!stat.isSymbolicLink()) {
      throw new Error(`saleem: ${link} exists and is not a symlink; remove it so saleem can manage the installation fallback`)
    }
