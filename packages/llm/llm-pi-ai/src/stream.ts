...
function classifyPiAiError(message: string): string {
  ...
  // Google surfaces a spent free-tier allowance as RESOURCE_EXHAUSTED; a
  // per-day metric is a spent quota, anything else is a per-minute rate limit.
  if (/resource[_\s-]?exhausted/i.test(message)) {
    return /\b(?:per\s?-?\s?day|daily|RPD|"?PerDay"?|GenerateRequestsPerDay)\b/i.test(message)
      ? QUOTA_EXCEEDED_CODE
      : 'RATE_LIMIT'
  }
