...
describe('createLayoutStore', () => {
  it('initializes the sidebar at its default width, details and tree closed, wide viewport assumed', () => {
  ...
  it('narrow toggleSidebar flips only the re-expand override; the width preference survives', () => {
    ...
    actions.toggleSidebar()
    expect(store.getSnapshot()).toEqual({
      sidebar: 400, details: 0, tree: 0, narrow: true, narrowExpanded: true,
    })
    actions.toggleSidebar()
  ...
  it('openTree uses the contract default, preserves an open width, and closeTree zeroes', () => {
    const { store, actions } = createLayoutStore().create()
    actions.openTree()
    expect(store.getSnapshot().tree).toBe(TREE_DEFAULT)
    actions.setTree(350)
    actions.openTree()
    expect(store.getSnapshot().tree).toBe(350)
    actions.closeTree()
