...
describe('ui-layout client apply', () => {
  ...
  it('provides ctx.layout and registers AppFrame into root with the four child declarations', async () => {
    ...
    expect(slots.spec('workspaceTree')).toEqual({ kind: 'single', scope: 'root' })
  })
  ...
  it('injects no business face and attaches the layout actions', async () => {
    ...
    await fiber.await()
    const actions = {
      setSidebar: vi.fn(), setDetails: vi.fn(), setTree: vi.fn(), toggleSidebar: vi.fn(),
      openDetails: vi.fn(), closeDetails: vi.fn(), openTree: vi.fn(), closeTree: vi.fn(),
    }
  ...
  it('teardown unwinds the service, the root registration, and the child declarations', async () => {
    ...
    expect(slots.spec('workspaceTree')).toBeUndefined()
