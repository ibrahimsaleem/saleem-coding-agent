...
Dynamic Cordis plugins temporarily extend the current Saleem Meta Harness process. A Plugin uses apply(ctx) to consume Services, listen to Events, provide Services, register model Tools, or register browser UI in Slots.
...
- Services, Events, Slots, Sessions, and their derived Cordis/Saleem Meta Harness objects are internal live data, not ordinary JSON that can be dumped.
...
- Host runs in the Saleem Meta Harness Node.js process and is appropriate for files, networking, commands, Agent/Session access, Host Events, Services, model Tools, and JSON methods callable by the Client.
