...
/**
 * Count words: contiguous Latin, numeric, or code tokens.
 * @param {string} body Markdown body.
 * @returns {{units: number, balanced: boolean, detailsCount: number, allCollapsed: boolean}} Visible word count and details shape.
 */
...
export function validateBody({
  ...
  if (count.detailsCount === 0) errors.push('the body must contain a default-collapsed <details> region')
  if (!count.allCollapsed) errors.push('<details> must be default-collapsed, without an open attribute')
  if (count.units > BODY_LIMIT) {
    errors.push(`the visible body is ${count.units} words, over the ${BODY_LIMIT}-word limit`)
  }
...
export function validateIssue(issue) {
  ...
  if (invalidLabels.length > 0) {
    errors.push(`an Issue must not use PR kind or legacy labels: ${invalidLabels.join(', ')}`)
  }
  if (
    ...
    errors.push('an Issue title must not carry a Type, Priority, Status, area, or Owner prefix')
  }
  if (!TYPES.has(issue.type ?? '')) errors.push('Type must be one of the five native Types')
  if (!status || !config.statuses.includes(status)) errors.push('an Issue must be in the Project with a valid Status')
  if (issue.priority !== null && !PRIORITIES.includes(issue.priority.toLowerCase())) {
    errors.push('Priority must be empty or one of P0-P3')
  }
  if (status === 'Done' && (issue.state !== 'closed' || issue.stateReason !== 'completed')) {
    errors.push('Done must map to the Completed close reason')
  }
  if (
    status === 'No action' &&
    (issue.state !== 'closed' || issue.stateReason !== 'not_planned')
  ) {
    errors.push('No action must map to the Not planned close reason')
  }
  if (!['Done', 'No action'].includes(status ?? '') && issue.state !== 'open') {
    errors.push(`${status} must map to an open Issue`)
  }
  return errors
}
...
export function validatePullRequest(input) {
  ...
  if (input.references.all.length === 0) errors.push('the PR body must reference at least one same-repository Issue')
  if (kinds.length !== 1) {
    errors.push(`a PR must have exactly one allowed kind/*, found ${kinds.length}`)
  }
  if (unknownKinds.length > 0) {
    errors.push(`the PR has unsupported kind/*: ${unknownKinds.join(', ')}`)
  }
  ...
  if (priorities.length > 1) errors.push(`a PR has at most one p0-p3, found ${priorities.length}`)
  if (areas.length === 0) errors.push('a PR must have at least one area/*')
  ...
  if (priorities.length === 0 && issuePriorities.length > 0) {
    ...
    errors.push(`the PR Priority should be ${highest}`)
    ...
    if (priorities[0] !== highest) errors.push(`the PR Priority should be ${highest}`)
  }
  return errors
}
...
async function projectContext(number, includeStatusActor = false) {
  ...
  if (!statusField) throw new Error('the Project has no Status field')
...
async function runPullRequestCheck(event) {
  ...
  if (errors.length > 0) {
    ...
    throw new Error(`Issue policy failed with ${errors.length} problem(s)`)
  }
  process.stdout.write(
    requiresPullRequestPolicy(pull) ? 'Issue policy passed.\n' : 'the PR is not yet in the Issue policy enforcement scope.\n',
  )
}
