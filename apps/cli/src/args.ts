/**
 * Commander adapter for the `saleem` command line.
 *
 ...
 * `saleem --profile tui --resume abc` boots the tui profile with `--resume abc`,
 * and `saleem --profile web -h` prints the web app's help, not this one's.
 *
...
const HELP_EXAMPLES = `
Examples:
  saleem --profile web                          boot the web profile (same as: saleem web)
  saleem --profile headless "run the tests"     answer one task, print the result, and exit
  saleem --profile tui --patch ./extra.yml      boot a custom profile with one extra overlay
  saleem --profile tui --resume <session>       arguments after the launcher flags reach the app
  saleem --profile web --help                   the web app's own flags and help
  saleem plugin --profile tui add <package>     install a plugin into the tui profile
`
...
export function parseDshArgs(argv: readonly string[], version: string): DshInvocation {
  ...
  program
    ...
    .description('saleem: boot a Saleem Meta Harness profile — an ordered stack of plugin-bundle patch layers under your own overrides.')
    .addHelpText('after', HELP_EXAMPLES)
    .exitOverride()
    // The launcher's flags come first and end at the first token it does not
    // know; everything from there on belongs to the booted app, including
    // its -h. `saleem -h` with no profile still prints this help, below.
    .helpOption(false)
    .allowUnknownOption()
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the booted profile\'s app (see: saleem --profile <name> --help)')
  ...
  web
    ...
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the web app (see: saleem web --help)')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
  ...
  if (resolved === undefined) throw new Error('saleem: no invocation resolved')
  return resolved
}
