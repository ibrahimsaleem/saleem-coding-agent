...
If the integration is missing because dependencies were restored from cache or `postinstall` was skipped, install it manually:
...
- `pre-commit` re-verifies the frozen archived Agent Note manifest when an archived note is staged, validates staged files with the project-free `.oxlintrc.staged.json` profile and applies Oxlint fixes with one bounded retry, regenerates `THIRD_PARTY_NOTICES.md` when a staged file is one of its inputs, checks the staged diff for whitespace errors, and runs the vendor manifest guard.
- `pre-merge-commit` re-verifies the frozen archived Agent Note manifest before Git creates an automatic merge commit.
